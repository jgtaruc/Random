//Juan Taruc CSCI 156 Project - UPD server
//This program will continually receive udp packets
//from the client for the specified received time.
...
class UDP_Server
{
   ...
   private static int received_time = 100000; //the specified time the server will repeatedly
                                            //received packets from the client in milliseconds.
   public static int timer_flag = 0; //indicates if the timer thread already started.
   public static int total_bits_received = 0; //the total bits received during the received time.
   
   //time thread simulates the time duration in which the server will allow
   ...
   {
      DatagramSocket serverSocket = new DatagramSocket(1234); //create a packet with the
